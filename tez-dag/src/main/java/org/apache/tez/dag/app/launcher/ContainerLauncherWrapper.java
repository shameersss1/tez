...
public class ContainerLauncherWrapper {
  ...
  public void vertexComplete(TezVertexID vertex, JobTokenSecretManager jobTokenSecretManager, Set<NodeId> nodeIdList) {
    if (real instanceof TezContainerLauncherImpl) {
      ((TezContainerLauncherImpl) real).vertexComplete(vertex, jobTokenSecretManager, nodeIdList);
    } else if (real instanceof LocalContainerLauncher) {
      ((LocalContainerLauncher) real).vertexComplete(vertex, jobTokenSecretManager, nodeIdList);
    }
  }
}
